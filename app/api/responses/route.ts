...
export async function POST(request: Request) {
  ...
  try {
    ...
    for (
      const [name, spec] of Object.entries(fields) as [
        string,
        { type: string; required: boolean; values?: string[]; max?: number },
      ][]
    ) {
      ...
      if (
        ...
        return Response.json({ error: 'Champ obligatoire', field: name }, { status: 400 });
      }
      ...
      if (spec.type === 'checkbox') {
        ...
        if (
          ...
          return Response.json({ error: 'Choix invalide', field: name }, { status: 400 });
        }
        clean[name] = values;
      } else {
        if (
          ...
          return Response.json({ error: 'Réponse invalide', field: name }, { status: 400 });
        }
